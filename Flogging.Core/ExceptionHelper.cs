...
namespace Flogging.Core
{
  public static class ExceptionHelper
  {
    public static string ToBetterString(this Exception ex, string prepend = null)
    {
      ...
      exceptionMessage.Append(GetExceptionDate("\n" + prepend, ex));
    ...
    private static string GetExceptionDate(string prependText, Exception exception)
    {
      ...
      {
        exData.Append(prependText + String.Format($"DATA-{key}:{exception.Data[key]}"));
      }
    ...
    private static string GetOtherExceptionProperties(Exception exception, string s)
    {
      ...
      var propertiesAlreadyHandled = new List<string> {
        "StackTrace", "Message", "InnerException",
        "Data", "HelpLink", "Source", "TargetSite" };
        ...
        .Where(prop => !propertiesAlreadyHandled.Contains(prop.Name)))
      {
        ...
        if (propEnumerable == null || propObject is string)
          allOtherProps.Append(s + String.Format($"{prop.Name} : {propObject}"));
        else { var enumerableSb = new StringBuilder();
          foreach (var item in propEnumerable)
          { enumerableSb.Append(item + "|"); }
          allOtherProps.Append(s + String.Format($"{prop.Name} : {enumerableSb}")); }
      }
